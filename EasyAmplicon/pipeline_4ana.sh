...
# 易扩增子EasyAmplicon
    ...
    # C:\0amplicon\EasyAmplicon
    # C:\0amplicon\EasyMicrobiome
...
### 2.3 改名后序列整合 integrate renamed reads
    ...
    tail -n 6 temp/all.fq|cut -c1-60
...
## 3. 切除引物与质控 Cut primers and quality filter
    ...
    tail temp/filtered.fa
    
    # # 批量对所有hmp样本每个序列改名
    # for i in `ls temp/*.merge.fa`
    # do
    # sample=`echo ${i}|awk -F "[.|/]" '{print $2}'`
    # echo $sample
    # time vsearch --fastx_filter ${i} \
    #   --fastq_stripleft 0 --fastq_stripright 0 \
    #   --relabel ${sample}. \
    #   --fastaout temp/${sample}.merge.relabel.fa
    # done
    # 
    # # 旧的filtered.fa 改为mysample，方便后续生成新的temp/filtered.fa
    # mv temp/filtered.fa temp/mysample.filtered.fa
    # 
    # # 合并所有hmp样本和自己样本的filtered.fa
    # cat temp/*.merge.relabel.fa temp/mysample.filtered.fa >temp/filtered.fa
...
### 4.3 基于参考去嵌合 Reference-based chimera detect
    ...
    # 可选数据库 silva-138-99-seqs.usearch.fa greengene2_2022.10.seqs.usearch.fna
...
## 8. 物种注释分类汇总
    ...
    head -n3 result/tax/sum_g.txt
     ...
     #无参 -- 没有参考数据库，鉴定ASV，我们会认为鉴定完的ASV是我们的参考数据库，
     #序列比对到数据库ASV上面，最后基于其他已知数据库对鉴定完的ASV进行注释
...
## 9. 有参定量特征表
    ...
    usearch -otutab temp/filtered.fa -otus ${db}/gg/97_otus.fa \
    	-otutabout result/gg/otutab.txt -threads 8
...
### 1.1 Alpha多样性箱线图
    ...
    for i in `head -n1 result/alpha/vegan.txt|cut -f 2-`;do
      ...
      Rscript ${db}/script/alpha_boxplot1.R --alpha_index ${i} \
        --input result/alpha/vegan.txt --design result/metadata.txt \
        --group Group --groupOrder ${plot_x_order} \
        --output result/alpha/ \
        --width 89 --height 59
    done
...
### 2.2 主坐标分析PCoA
    ...
    Rscript ${db}/script/beta_pcoa2.R \
      --input result/beta/bray_curtis.txt --design result/metadata.txt \
      --group Group --width 89 --height 59 \
      --output result/beta/bray_curtis.pcoa.boxplot.group.pdf
### 2.3 限制性主坐标分析CPCoA
...
### 3.1 堆叠柱状图Stackplot
    ...
    Rscript ${db}/script/tax_stackplot1.R \
      ...
      --color manual1 \
      --groupOrder ${plot_x_order}
    # Rscript ${db}/script/tax_stackplot1.R \
    #   --input result/tax/sum_${i}.txt --design result/metadata.txt \
    #   --group Time --color manual1 \
    #   --output result/tax/sum_${i}.stackplot.sub_stage \
    #   --legend 20 --width 200 --height 150 --subgroup Group\
    #   --groupOrder ${plot_x_order}
    done
...
### 3.2 弦/圈图circlize
    ...
    for i in p c o f g; do
    Rscript ${db}/script/tax_circlize_xlim90.R \
      --input result/tax/sum_${i}.txt --design result/metadata.txt \
      --group Group  --output result/tax/sum_${i}.circlize \
...
### 1.3 热图
    ...
    bash ${db}/script/compare_heatmap1.sh -i result/compare/${compare}.txt -l 7 \
       -d result/metadata.txt -A Group \
       -w 8 -h 5 -s 7 \
       -o result/result/compare/${compare}
...
### 1.5 单个特征的绘制
    ...
    awk '$4<0.05' result/compare/${compare}.txt | sort -k7,7nr | cut -f1 | head
    # 差异OTU细节展示
...
## 1.6 按照物种水平计算差异
    ...
    echo $compare
    Rscript ${db}/script/compare.R -s F \
      --input result/tax/sum_${tax}.txt --design result/metadata.txt \
      --group Group --compare ${compare} --threshold 0 \
      --method edgeR --pvalue 0.05 --fdr 1 \
      --output result/compare/${tax}-
      
    Rscript ${db}/script/compare_volcano1.R \
      --input result/compare/${tax}-${compare}.txt \
      --compare ${compare} \
      --output result/compare/${tax}-${compare}.volcano.pdf \
      --width 120 --height 90
...
### PICRUSt 1.0 (此处分析适合从网站上进行PICRUSt分析，然后下载结果到自己的resultd的picrust文件夹)
...
## 2. 元素循环FAPROTAX
    ...
    # 设置工作目录
    wd=/mnt/c/0amplicon/EasyAmplicon/result/faprotax/
    mkdir -p ${wd} && cd ${wd}
    # 设置脚本目录
    sd=/mnt/c/0amplicon/EasyMicrobiome/script/FAPROTAX_1.2.7
...
# 32、MachineLearning机器学习
    ...
    conda activate qiime2-amplicon-2024.10
    cd /mnt/c/0amplicon/EasyMicrobiome/script/slime2
    ...
    ./slime2.py ${wd}/result/otutab.txt ${wd}/result/metadata.txt --normalize --tag rf_e4 rf -n 10000
...
## 1. 筛选高丰度/指定的特征
    ...
    #方法3 基于差异结果或机器学习拿到一些otu名字 存到enriched_ID.txt 
    # seqkit、python的代码、shell代码
    # usearch -fastx_getseqs ../otus.fa -labels enriched_ID.txt -fastaout otus.fa
...
## 1. LEfSe分析
    ...
    ## input.res结果第一列为样本名（层级结构用点分割），第二列为丰度最大值的log10，
    ## 第三列为有差异物种的分组信息（无差异为空），第四列为有差异物种的LDA值（无差异为空），
    ## 第五列为有差异物种的pvalue值（无差异为-）
    
    #绘制物种树注释差异
    lefse-plot_cladogram.py input.res cladogram.pdf --format pdf
    ## dpi会增大图形圈大小 
    ## labeled_stop_lev指定legend展示的物种等级 
    ## abrv_stop/start_lev 指定图中差异物种背景色展示的等级
...
## 2. PICRUSt功能预测
    ...
    wd=/mnt/c/0amplicon/EasyAmplicon/ #linux wd=/mnt/
    cd $wd/result/gg
    # 启动环境
    ...
    # 设置数据库目录，如 /mnt/d/db
    db=/mnt/c/0amplicon/EasyMicrobiome
    #校正拷贝数，30s, 102M
    ...
    metagenome_contributions.py -i otutab_norm.biom -l K01362,K01104 \
       -o metagenome_contributions.txt \
       -c ${db}/gg/ko_13_5_precalculated.tab.gz
    
    cp metagenome_contributions.txt ../picrust
    cp ko.txt ../picrust/all_level.ko.txt 
    l=L1;cp pathway1.txt ../picrust/all_level.ko.${l}.txt 
    l=L2;cp pathway2.txt ../picrust/all_level.ko.${l}.txt 
    l=L3;cp pathway3.txt ../picrust/all_level.ko.${l}.txt 
    
    # 对功能预测结果进行差异分析以及可视化 ---请切换到git for windows里操作
    ## 把文件重命名并移动到picrust文件夹下为了和之前的代码兼容
    l=L2
    sed '/# Const/d;s/OTU //' result/picrust/all_level.ko.${l}.txt > result/picrust/${l}.txt
    num=`head -n1 result/picrust/${l}.txt|wc -w`
    paste <(cut -f $num result/picrust/${l}.txt) <(cut -f 1-$[num-1] result/picrust/${l}.txt) \
    ...
    awk 'BEGIN{OFS=FS="\t"}ARGIND==1{name=$2;gsub("; ","\t",name);a[$1]=name}\
    ARGIND==2&&FNR==1{print $0}ARGIND==2&&FNR>1{print $0,a[$3]}' ${db}/gg/97_otu_taxonomy.txt \
    result/picrust/metagenome_contributions.txt > result/picrust/metagenome_contributions_tax.txt 
    
    head result/picrust/metagenome_contributions_tax.txt
    
    Rscript ${db}/script/plot_metagenome_contributions.R \
      --input result/picrust/metagenome_contributions_tax.txt \
      --output result/picrust/K01362_contrib.pdf \
      --function_id K01362 --rank Phylum \
      --width 10 --height 10
...
## 3. Bugbase细菌表型预测
    ...
    # 进入 git for windows
    #安装依赖包
    export BUGBASE_PATH=/c/0amplicon/EasyMicrobiome/script/BugBase
    ...
    run.bugbase.r -i gg/otutab.txt -m MappingFile.txt -c Group -o phenotype/
...
## 2.1 样本序列文件为fasta文件解决方法：
    # fasta文件转成fastq文件(必须，并且质量值不能重复)
    for i in `ls Rawdata-tags-renamed/*`;do ls $i;
    sample=`ls $i|cut -f2 -d "/"|cut -f1-3 -d "."`
    awk '{if($0~/>/) {sub(">","@");print $0}\
    else {qv=$0;gsub("A","F",qv);gsub("T","G",qv);gsub("C","H",qv);gsub("G","I",qv);print $0"\n+\n"qv;}}' $i >seq/${sample}.fq
    
    vsearch --fastq_convert seq/${sample}.fq --relabel ${sample}_ --fastqout seq/${sample}_rename.fq
    done
