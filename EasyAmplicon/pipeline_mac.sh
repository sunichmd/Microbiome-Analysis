...
# 易扩增子EasyAmplicon
    ...
    # Mac 与 win 的设置不同
    bin=${db}/mac
    chmod 755 ${bin}/* #给权限 7（所有者 全部权限）5（同组的人 可写可执行权限）5（其他人 可写可执行权限） 
    sed -i 's/\r//g' ${db}/script/*.sh # 把windows换行符删除掉 *.sh 表示 所有以sh结尾的文件 *任意字符
    ...
    export PATH=${bin}:`pwd`/${bin}:${db}/script:${PATH} # 设置环境变量 保证我们的vsearch、script文件夹里的脚本后续可以使用 
    cd ${wd}
...
### 1.1. 元数据/实验设计 metadata
    ...
    cat -vet result/metadata_raw.txt | head -n3
    # windows用户结尾有^M，运行sed命令去除，再用cat -A检查
    sed 's/\r//' result/metadata_raw.txt > result/metadata.txt
    cat -vet result/metadata.txt | head -n3
    
    ## 对于自己的metadata.txt 跳过1.1即可；但要保证生成了metadata.txt
...
### 1.2. 测序数据 sequencing data
    ...
    ls -sh seq/ #list seq文件夹的内容 -sh 并输出size+human read
...
### 2.1 合并双端序列并按样品重命名 Merge pair-end reads and rename
    ...
    # ctrl + shift +c
    time vsearch --fastq_mergepairs seq/WT1_1.fq.gz --reverse seq/WT1_2.fq.gz --fastqout temp/WT1.merged.fq  --relabel WT1.
    head temp/WT1.merged.fq

    # \ 把一些有意义的符号转换成没有意义的符号 本来回车是执行的代码的意思 \+enter 普通的换行
...
### 5.1 生成特征表
    ...
    gsed -i 's/\r//' result/raw/otutab.txt
    head -n6 result/raw/otutab.txt
    head -n6 result/raw/otutab.txt | cut -f 1-6 |cat -vet
    # csvtk统计表行列
...
### 5.2 物种注释，且/或去除质体和非细菌 Remove plastid and non-Bacteria
    ...
    head result/raw/otus.sintax | cat -vet
    gsed -i 's/\r//' result/raw/otus.sintax
    ...
    awk 'NR==FNR{a[$1]=$0}NR>FNR{print a[$1]}'\
        ...
        > result/otus.sintax
    #补齐末尾列
    gsed -i 's/\t$/\td:Unassigned/' result/otus.sintax
...
### 5.3 等量抽样标准化
    ...
    ${db}/mac/summary.sh -f result/otutab_rare.txt -c CTCTCT
...
### 6.1. 计算α多样性 calculate alpha diversity
    ...
    # 或用下面的R脚本计算6种多样性值
    ${db}/script/compute_alpha.R --input result/otutab_rare.txt \
      --depth 0 \
      --output result/alpha/alpha.txt
...
### 6.2. 计算稀释丰富度 calculate rarefaction richness
    ...
    # 方法2
    Rscript ${db}/script/alpha_rare_calculate.R --input result/otutab_rare.txt \
    --seed 1 --alpha_index richness --output result/alpha/alpha_rare.txt
...
## 7. β多样性 Beta diversity
    ...
    # 使用fasttree构建进化树
    muscle -in result/otus.fa -out result/otus_aligned.fas
    fastTree -gtr -nt result/otus_aligned.fas > result/otus.nwk
    # 可以选择的距离：unifrac，"manhattan", "euclidean", "canberra", "bray", 
		#"kulczynski", "gower", "morisita", "horn", "mountford", 
		#"jaccard", "raup", "binomial", "chao", "altGower", "cao", 
		#"mahalanobis", "clark", "chisq", "chord", "hellinger", 
		#"aitchison", "robust.aitchison"
		# 如果beta_index设置all 就是计算 unifrac、euclidean、jaccard、bray_curtis
    Rscript ${db}/script/beta_calculate.R -h
    Rscript ${db}/script/beta_calculate.R --input result/otutab_rare.txt \
    --beta_index all --tree result/otus.nwk \
    --output result/beta
...
## 8. 物种注释分类汇总
    ...
    for i in p c o f g s;do
      ${db}/script/sintax_summary.R -t result/taxonomy.txt -i result/otutab_rare.txt --rank ${i} \
   ...
   # vesearch 鉴定ASV结束，无参的分析

## 9. 有参定量特征表 另一套流程 有参的分析
...
### 1.1 Alpha多样性箱线图
    ...
    mv alpha_boxplot_TukeyHSD.txt result/alpha/alpha_pvalue-23-25.txt
...
### 2.2 绘制扩展柱状图和表
    ...
    Rscript ${db}/script/compare_stamp.R \
      ...
      --group Group --compare ${compare} --threshold 0.1 \
      --method "wilcox" --pvalue 0.05 --fdr "none" \
...
### PICRUSt 2.0
    ...
    # 进入工作目录，服务器要修改工作目录
    wd=/mnt/c/0amplicon/EasyAmplicon/
    wd1=${wd}/result/picrust2
    mkdir -p ${wd1} && cd ${wd1}
    # 运行流程，内存15.7GB，耗时12m
...
# 32、MachineLearning机器学习
    ...
    ./slime2.py otutab.txt design.txt --tag ab_e4 ab -n 10000
    #使用RandomForest计算10000次(14.5s)，推荐百万次，支持多线程
    ./slime2.py otutab.txt design.txt --normalize --tag rf_e4 rf -n 10
...
## 10. windows换行符查看和删除
    ...
    # 查看行尾是否有^M
  	cat -vet metadata.txt
  	# 删除^M，并写入新文件
  	sed 's/\r//' metadata.txt > metadata.mod.txt
  	# 检查是否成功
  	cat -vet metadata.mod.txt
